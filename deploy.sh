...
# 6. 配置 GitHub Secrets
echo "🔐 配置 Metabase 凭据 Secrets..."
gh secret set METABASE_URL --body "https://metabase.pixcakeai.com" --repo "$GH_USER/$REPO_NAME"
gh secret set METABASE_USER --body "jbr@pixcakeai.com" --repo "$GH_USER/$REPO_NAME"
gh secret set METABASE_PASS --body "jdsiT9QmzPzuzp" --repo "$GH_USER/$REPO_NAME"

# 7. 开启 GitHub Pages（用 Actions 作为 source）
...
echo ""
echo "每 30 分钟自动从 Metabase 拉一次最新数据并部署，"
